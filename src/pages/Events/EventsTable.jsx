...
import EditEvent from "../../components/Modals/EventModals/EditEvent";
import BootstrapTable from 'react-bootstrap-table-next';
import paginationFactory from 'react-bootstrap-table2-paginator';
import DeleteSeveralEvents from "../../components/Modals/EventModals/DeleteSeveralEvents";
import DeleteEvent from "../../components/Modals/EventModals/DeleteEvent";
...
const EventsTable = () => {
    ...
    const columns = [{
        ...
        formatter: (cellContent, row) => {
            const formatDate = row.beginning_date.split("-")
        ...
        formatter: (cellContent, row) => {
            const formatDate = row.ending_date.split("-")
            return `${formatDate[2]}-${formatDate[1]}-${formatDate[0]}`
        }
    }, {
        dataField: 'delete',
        ...
        formatter: (cellContent, row) => {
            return (
                <DeleteEvent id_event={row.id_event} eventsToDelete={idToDelete}/>
            );
        },
    }, {dataField: 'edit',
        text: '',
        formatter: (cellContent, row) => {
            return (
                <EditEvent data={row} id={row.id_event} />
            );
        },
    }]
    ...
    return (
        <div className={`${styles.table_main}`}>
            <DeleteSeveralEvents eventsToDelete={idToDelete} />
            <BootstrapTable
